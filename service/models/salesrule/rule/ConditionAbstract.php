<?php
namespace service\models\salesrule\rule;
 ...
 * @package service\models\salesrule\rule
 * @method string getAttribute()
 * @method setAttribute(string $attr)
 */
...
//        ));
    ...
    public function validateAttribute($validatedValue)
    {
        ...
        foreach ($value_list as $index => $value) {
            //Mage::log("validateOneValue($validatedValue, $value, $op);", null, 'ruleTest.log');
            $result = $this->validateOneValue($validatedValue, $value, $op);
            //Mage::log("result:$result", null, 'ruleTest.log');
            if ($result) {
                // 若符合，则记下当前级数
                $level = $index;
            } else {
        ...
        //Mage::log("level:$level", null, 'ruleTest.log');
        // index从0开始，所以返回+1的值
        if ($level != -1) {
            return $level + 1;
        } else {
